...
describe("persistence > many-to-one uni-directional relation", function() {
    ...
    it("should save a category with a post attached", () => Promise.all(connections.map(async connection => {
        ...
        const loadedCategory = await connection.manager.findOne(qr, Category, 1, { relations: ["post"] });
    ...
    it("should update exist post by cascades when category is saved", () => Promise.all(connections.map(async connection => {
        ...
        const loadedCategory1 = await connection.manager.findOne(qr, Category, 1, { relations: ["post"] });
        ...
        await connection.manager.save(qr, loadedCategory1);

        const loadedCategory2 = await connection.manager.findOne(qr, Category, 1, { relations: ["post"] });
