...
describe("github issues > #970 Mongo Bad Sort Specification", () => {
    ...
    it("should order properly without errors", () => Promise.all(connections.map(async connection => {
        ...
        const loadedPosts1 = await postRepository.find(qr, { where: { title: "Post" }, order: { text: 1 } });
        ...
        const loadedPosts2 = await postRepository.find(qr, { where: { title: "Post" }, order: { text: "ASC" } });
        ...
        const loadedPosts3 = await postRepository.find(qr, { where: { title: "Post" }, order: { text: -1 } });
        ...
        const loadedPosts4 = await postRepository.find(qr, { where: { title: "Post" }, order: { text: "DESC" } });
