...
describe("github issue #1754 Repository.save(qr) always updating ManyToOne relation", () => {
    ...
    it("should work as expected", () => Promise.all(connections.map(async connection => {
        ...
        await connection.manager.save(qr, tipoCliente1);

        const tipoCliente2 = new TipoCliente();
        tipoCliente2.id = 2;
        tipoCliente2.descricao = "XXXX";
        await connection.manager.save(qr, tipoCliente2);

        const cliente = new Cliente();
        cliente.id = 1;
        cliente.nome = "Kirliam";
        cliente.tipo = tipoCliente1;
        await connection.manager.save(qr, cliente);
        ...
        await connection.manager.getRepository(Cliente).save(qr, myReceivedJson1);
        ...
        let clienteDb1: Cliente = await connection.manager.getRepository(Cliente).preload(qr, myReceivedJson2) as Cliente;
        await connection.manager.getRepository(Cliente).save(qr, clienteDb1);

        let myReceivedJson3 = {id: 1, nome: "Kirliam changed 3", tipo: {id: 2, descricao: "XXXX"}};
        await connection.manager.getRepository(Cliente).save(qr, myReceivedJson3);
