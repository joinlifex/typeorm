...
describe("github issues > #2199 - Inserting value for @PrimaryGeneratedColumn() for mysql, sqlite and mssql", () => {
    ...
    it("should allow to explicitly insert primary key value", () => Promise.all(connections.map(async connection => {
        ...
        const firstBarQuery =  connection.manager.create(qr, Bar, {
             id: 10,
            description: "forced id value"
        });
        const firstBar = await connection.manager.save(qr, firstBarQuery);
        ...
        const secondBarQuery =  connection.manager.create(qr, Bar, {
            description: "default next id value"
        });
        const secondBar = await connection.manager.save(qr, secondBarQuery);
        ...
        const thirdBarQuery =  connection.manager.create(qr, Bar, {
            id: 5,
            description: "lower forced id value"
        });
        const thirdBar = await connection.manager.save(qr, thirdBarQuery);
    ...
    it("should reset mssql's INSERT_IDENTITY flag correctly after failed queries", () => Promise.all(connections
        ...
        .map(async connection => {
            ...
            const successfulBarQuery = connection.manager.create(qr, Bar, {
                description: "default id value"
            });
            const bar = await connection.manager.save(qr, successfulBarQuery);
