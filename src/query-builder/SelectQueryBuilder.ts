...
export class SelectQueryBuilder<Entity> extends QueryBuilder<Entity> implements WhereExpressionBuilder {
     ...
     */
    async getRawOne<T = any>(queryRunner: QueryRunner): Promise<T|undefined> {
        return (await this.getRawMany(queryRunner))[0];
    }

    /**
     * Gets all raw results returned by execution of generated query builder sql.
     */
    async getRawMany<T = any>(queryRunner: QueryRunner): Promise<T[]> {
     ...
     */
    async getRawAndEntities<T = any>(queryRunner: QueryRunner): Promise<{ entities: Entity[], raw: T[] }> {
        let transactionStartedByUs: boolean = false;
        try {
     ...
     */
    async getOne(queryRunner: QueryRunner): Promise<Entity|undefined> {
        const results = await this.getRawAndEntities(queryRunner);
     ...
     */
    async getOneOrFail(queryRunner: QueryRunner): Promise<Entity> {
        const entity = await this.getOne(queryRunner);
     ...
     */
    async getMany(queryRunner: QueryRunner): Promise<Entity[]> {
        if (this.expressionMap.lockMode === "optimistic")
            throw new OptimisticLockCanNotBeUsedError();
     ...
     */
    async getCount(queryRunner: QueryRunner): Promise<number> {
        if (this.expressionMap.lockMode === "optimistic")
            throw new OptimisticLockCanNotBeUsedError();
     ...
     */
    async getManyAndCount(queryRunner: QueryRunner): Promise<[Entity[], number]> {
        if (this.expressionMap.lockMode === "optimistic")
            throw new OptimisticLockCanNotBeUsedError();
     ...
     */
    async stream(queryRunner: QueryRunner): Promise<ReadStream> {
        this.expressionMap.queryEntity = false;
    ...
    protected async executeEntitiesAndRawResults(queryRunner: QueryRunner): Promise<{ entities: Entity[], raw: any[] }> {
        ...
        if ((this.expressionMap.skip || this.expressionMap.take) && this.expressionMap.joinAttributes.length > 0) {
            ...
            rawResults = await new SelectQueryBuilder(this.connection)
        ...
        if (rawResults.length > 0) {
            ...
            const rawRelationIdResults = await relationIdLoader.load(queryRunner, rawResults);
            const rawRelationCountResults = await relationCountLoader.load(queryRunner, rawResults);
