...
createConnection(options).then(async connection => {
    ...
    const loadedEmployee = await employeeRepository.findOne(qr, 1);
    ...
    await homesitterRepository.save(qr, homesitter);
    console.log("homesitter has been saved: ", homesitter);

    console.log("now loading the homesitter: ");
    const loadedHomesitter = await homesitterRepository.findOne(qr, 2);
    ...
    await studentRepository.save(qr, student);
    console.log("student has been saved: ", student);

    console.log("now loading the student: ");
    const loadedStudent = await studentRepository.findOne(qr, 3);
    console.log("loaded student: ", loadedStudent);

    console.log("-----------------");
    const secondEmployee = await employeeRepository.findOne(qr, 2);
    console.log("Non exist employee: ", secondEmployee);
    const thirdEmployee = await employeeRepository.findOne(qr, 3);
    console.log("Non exist employee: ", thirdEmployee);
    console.log("-----------------");
    const secondHomesitter = await homesitterRepository.findOne(qr, 1);
    console.log("Non exist homesitter: ", secondHomesitter);
    const thirdHomesitter = await homesitterRepository.findOne(qr, 3);
    console.log("Non exist homesitter: ", thirdHomesitter);
    console.log("-----------------");
    const secondStudent = await studentRepository.findOne(qr, 1);
    console.log("Non exist student: ", secondStudent);
    const thirdStudent = await studentRepository.findOne(qr, 2);
